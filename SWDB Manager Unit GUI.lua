...
unitTypeLabels = {[0] = {[0] = "managed", "unitType"},
{[0] = "managed", "unitType", "mainType", "subType", "name", "nameLength"},
{[0] = "managed", "unitType", "targetNumber", "targetActive", "targetX", "targetY", "targetZ"},
}
...
function onDraw()
	...
	i = 1
	screen.drawText(2, 6 * i + #labels * 6 - 2, "Managed Units:" .. managedUnitsCount)

	managedUnitsAddresses = "Managed units include:   "
	for key, value in pairs(managedUnits) do
		if value[0] then
			managedUnitsAddresses = managedUnitsAddresses .. key .. "; "
		end
	end

	i = 2
	screen.drawText(2, 6 * i + #labels * 6 - 2, managedUnitsAddresses)

	managedWeaponsAddresses = "Managed weapons include: "
	for key, value in pairs(managedUnits) do
		if value[0] and (value[1] == 1) then
			managedWeaponsAddresses = managedWeaponsAddresses .. key .. "; "
		end
	end
	...
	unitSelectedType = unitSelectedData[1] or -1
	...
	for key, value in pairs(unitSelectedData) do
		...
		unitTypeLabels[key] .. "=" .. ((type(value) == "boolean" or type(value) == "table") and (value and "true" or "false") or value) .. "\n"
	end
	i = 5
